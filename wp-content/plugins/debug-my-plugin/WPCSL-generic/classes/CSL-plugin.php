 ...
 *
 **/
if (!defined('WPCSL____VERSION')) { define('WPCSL____VERSION', '2.1.5'); }
...
class wpCSL_plugin__ {
     ...
     * 
     * @var wpCSL_notifications__ $notifications 
     */
     ...
     *
     * @param mixed[] $params a named array where key is the string of a wpCSL_plugin__ property, key is the initial value.
     */
    ...
    function create_helper($class = 'none') {
        switch ($class) {
            ...
            case 'default':
            default:
                $this->helper = new wpCSL_helper__($this->helper_config);
    ...
    function create_notifications($class = 'none') {
        ...
        $this->notifications = 
            new wpCSL_notifications__($this->notifications_config);
    }
    ...
    function create_products($class = 'none') {
        switch ($class) {
            ...
            case 'wpCSL_products__':
            case 'default':
            default:
                $this->products = new wpCSL_products__($this->products_config);
    ...
    function create_settings($class = 'none') {
        switch ($class) {
            ...
            case 'wpCSL_settings__':
            case 'default':
            default:
                $this->settings = new wpCSL_settings__($this->settings_config);
    ...
    function create_themes($class = 'none') {
        ...
        $this->themes = new wpCSL_themes__($this->themes_config);
    }    
    ...
    function create_license($class = 'none') {
        switch ($class) {
            ...
            case 'wpCSL_license__':
            case 'default':
            default:
                if ($this->has_packages || !$this->no_license) {
                    $this->license = new wpCSL_license__($this->license_config);
                }
    ...
    function create_cache($class = 'none') {
        switch ($class) {
            ...
            case 'default':
            default:
                $this->cache = new wpCSL_cache__($this->cache_config);
    ...
    function add_wp_actions() {
        ...
        //
            ...
            if (($this->debugging) && ($this->driver_type != 'none')) {
                print __('DEBUG: No driver found.',WPCSL____VERSION);
            }
        }
    }
    ...
    function add_meta_links($links, $file) {
        ...
        if ($file == $this->basefile) {
            if (isset($this->support_url)) {
                $links[] = '<a href="'.$this->support_url.'" title="'.__('Support',WPCSL____VERSION) . '">'.
                            __('Support',WPCSL____VERSION) . '</a>';
            }
            if (isset($this->purchase_url)) {
                $links[] = '<a href="'.$this->purchase_url.'" title="'.__('Purchase',WPCSL____VERSION) . '">'.
                            __('Buy Now',WPCSL____VERSION) . '</a>';
            }
            $links[] = '<a href="options-general.php?page='.$this->prefix.'-options" title="'.
                            __('Settings',WPCSL____VERSION) . '">'.__('Settings',WPCSL____VERSION) . '</a>';
        }
        return $links;
    }
    ...
    function load_driver() {
        ...
        //
            ...
            //
                ...
                if ($this->debugging) {
                    print __('DEBUG: could not locate driver:',WPCSL____VERSION) . 
    ...
    function add_display_settings() {      
        $this->settings->add_section(array(
                'name' => __('Display Settings',WPCSL____VERSION),
                'description' => '',
                ...
                if (exec('locale -a', $locales)) {
                    ...
                    $this->settings->add_item(
                        ...
                        __('Sets the locale for PHP program processing, affects time and currency processing. '.
                            'If you change this, save settings and then select money format.',WPCSL____VERSION),
                        $locale_custom
                    );
                }
            } else {
                    $this->settings->add_item(
                        ...
                        null, 
                        false, 
                        __('Your PHP settings have disabled exec(), your locale list cannot be determined.',WPCSL____VERSION),
                        '&nbsp;'
                    );
            }
        }            
        ...
        if  (
            ...
            (($this->driver_type == 'Panhandler') || $this->uses_money) && (function_exists('money_format')) 
                ...
                $this->settings->add_item(
                    ...
                    'list', 
                    false, 
                    __('This is based on your current locale, which is set to ',WPCSL____VERSION).
       ...
       if (isset($this->rate_url)){
        	...
        	$this->settings->add_item(
        		...
        		'checkbox', 
        		false, 
        		__('This will disable the notification asking you to rate our product.',WPCSL____VERSION)
        		);
            ...
            if($this->settings->get_item('thisbox')==false){
                ...
                if ($time < $destruct_time){
                    ...
                    $this->notifications->add_notice(
                        ...
                        This message will self destruct in: '.$hours_remaining,WPCSL____VERSION)                        
                    );
                }
    ...
    function shortcode_show_items($atts, $content = NULL) {
        if ( $this->ok_to_show() ) {
            ...
            if ($this->debugging) {
                if (is_array($atts)) {
                    print __('DEBUG: Shortcode called with attributes:',WPCSL____VERSION) . "<br/>\n";
                    foreach ($atts as $name=>$value) {
                        print $name.':'.$value."<br/>\n";
                    }
                } else {
                    print __('DEBUG: Shortcode called with no attributes.',WPCSL____VERSION) . "<br/>\n";
                }
            }            
        ...
        // Not OK TO Show
            ...
            if ($this->debugging) {
                $content = __('DEBUG: Not OK To Show',WPCSL____VERSION);
            }
        }
        return $content;
    }
    ...
    function render_object_list($atts) {
        ...
        //
            ...
            $content= __('No products found',WPCSL____VERSION);
        }
