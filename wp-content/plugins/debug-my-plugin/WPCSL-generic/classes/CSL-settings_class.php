...
class wpCSL_settings__ {
    ...
    function add_section($params) {
        if (!isset($this->sections[$params['name']])) {
            $this->sections[$params['name']] = new wpCSL_settings_section__(
                array_merge(
                    $params,
        ...
        if (!isset($this->sections[$section])) {
            if (isset($this->notifications)) {
                $this->notifications->add_notice(
                    3,
                    sprintf(
                       __('Program Error: section <em>%s</em> not defined.',WPCSL____VERSION),
                       $section
                       )
                );
            }
 ...
 */
class wpCSL_settings_section__ {
    ...
    function add_item($params) {
        $this->items[] = new wpCSL_settings_item__(
            array_merge(
                $params,
                array('plugin_url' => $this->plugin_url,
                      'css_prefix' => $this->css_prefix,
